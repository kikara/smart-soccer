...
import AudioEvent from "../../game/AudioEvent";
...
export default {
    ...
    methods: {
        ...
        eventHandle(event, state) {
            ...
            this.audioEvent.handleEvent(event, state);
        },
        async booked(state) {
            await this.setGamers(state);

            this.gamersFetched = true;
        ...
        async setGamers(state) {
            for (const item of Object.values(state.round.gamers)) {
                await axios.get('/api/users/' + item.user_id).then((response) => {
